...
<!--    <xsl:include href="commons/params.xsl"/>-->
    ...
    <!-- Maintenance note: For each revision, change the content of &lt;recordInfo&gt;&lt;recordOrigin&gt; to reflect the new revision number.
			...
			Revision 2.03 - Added if tests to originInfo producer elements to avoid empty tags.  2022/11/08
    ...
    <xsl:template match="/">
           <xsl:param name="workingDir" select="'C:/Users/Carlos.Martinez/OneDrive%20-%20USDA/Desktop/NAL-RDA-MARC21slim2-mods/xml/BIBLIOGRAPHIC_13616235400007426_1/'"/>
           <xsl:param name="originalFilename" select="replace(saxon:system-id(),'(.*/)(.*)(\.xml)','$2')"/>
            <modsCollection xmlns="http://www.loc.gov/mods/v3"
                ...
                <xsl:result-document version="1.0" encoding="UTF-8" method="xml" indent="yes" media-type="text/xml"
                    format="archiveOriginal"
    ...
    <xsl:template match="*" name="extension">
...
-->
        ...
        <originInfo>
            ...
            <!-- Revision 2.02 apply marc country conversion function -->

            <xsl:variable name="MARCpublicationCode2">
                <xsl:analyze-string
                    select="replace($controlField008, '(\d+[a-z]\d+)(\D{2,3})', '$2')"
                    regex="(\d+[a-z]\d+)([a-z]{{2,3}})">
                    <xsl:matching-substring>
                        <xsl:if test="matches(., '[a-z]{2,3}')">
                            <xsl:value-of select="normalize-space(regex-group(2))"/>
                        </xsl:if>
                    </xsl:matching-substring>
                    <xsl:non-matching-substring>
                        <xsl:variable name="MARCpublicationCode"
                            select="normalize-space(translate(substring($controlField008, 16, 3), '|', ' '))"/>
                        <xsl:if test="contains($MARCpublicationCode, '0123455789')">
                            <xsl:value-of
                                select="translate($MARCpublicationCode, '0123456789', ' ')"/>
                            <place>
                                <placeTerm>
            ...
            <xsl:if test="matches($MARCpublicationCode2, '[a-z]{2,3}')">
                <place>
                    <!-- marccountry code -->
                    <placeTerm>
                        <xsl:attribute name="type">code</xsl:attribute>
                        <xsl:attribute name="authority">marccountry</xsl:attribute>
                        <xsl:value-of select="$MARCpublicationCode2"/>
                    </placeTerm>
                    <!-- decodes MARC Country Codes -->
                    <placeTerm>
                        <xsl:attribute name="type">text</xsl:attribute>
                        <xsl:value-of select="f:convertMARCCountry($MARCpublicationCode2)"/>
                    </placeTerm>
                </place>
            </xsl:if>
            ...
            <xsl:variable name="controlfield008-7-12">
                <xsl:analyze-string select="replace($controlField008,'(\d+)([a-z])(.*)','$3')" regex="(\d+)(.*)">
                    <xsl:matching-substring>
    ...
    -->
        ...
        <xsl:for-each select="datafield[@tag = 264][@ind2 = 0]">
            <!-- Revision 2.03 added if test to originInfo producer cm3 2022/10 -->
        ...
        <xsl:choose>
            <xsl:when test="matches(translate($controlField008-36-38, '|', ' '), '[a-z]{3}')">
                <language>
...
-->
                    ...
                    <xsl:analyze-string select="." regex="(\d{{4,6}}.\w{{0,4}})(.+)">
                        <xsl:matching-substring>
                            ...
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:if test="matches(., '(.*p.)(\d{1,7})\-')">
                                        <extent unit="pages">
                                            <total>
                                                <xsl:value-of
                                                  select="replace(regex-group(2), '(.*p.)(\d{1,7})\-', '$2')"
                                                />
                                            </total>
                                        </extent>
                                    </xsl:if>
    ...
    <!--<xd:doc>
        <xd:desc>Gets both issn and agid from 914</xd:desc>
    </xd:doc>
    <xsl:template name="relatedIdentifierISSNandLocal" match="datafield[@tag = 914]">
        <xsl:if test="datafield[@tag = 914]/subfield[@code = 'a' or code = 'b']">
            <identifier type="issn">
                <xsl:value-of select="datafield[@tag = 914]/subfield[@code = 'b']"/>
            </identifier>
            <identifier type="local">
    ...
    </xsl:template>	<xd:doc id="subject653Type" scope="component">
    ...
    </xsl:template>	<xd:doc id="xxx880" scope="component">
...
-->
    ...
    <!-- titleInfo 130 730 245 246 240 740 210 -->	<xd:doc id="createTitleInfoFrom130" scope="component">
		<xd:desc> 130 </xd:desc>
	</xd:doc>
    ...
    </xsl:template>	<xd:doc id="createTitleInfoFrom245" scope="component">
    ...
    </xsl:template>	<xd:doc id="createTitleInfoFrom240" scope="component">
    ...
    </xsl:template>	<xd:doc id="createTOCFrom505" scope="component">
    ...
    </xsl:template>	<xd:doc id="createAbstractFrom520" scope="component">
    ...
    </xsl:template>	<xd:doc id="createTargetAudienceFrom521" scope="component">
    ...
    </xsl:template>	<xd:doc id="createNoteFrom245c" scope="component">
    ...
    </xsl:template>	<xd:doc id="createSubGeoFrom034" scope="component">
    ...
    </xsl:template>	<xd:doc id="createLocationFrom852" scope="component">
    ...
    </xsl:template>	<xd:doc id="createAccessConditionFrom506" scope="component">
    ...
    </xsl:template>	<xd:doc id="createNameFrom910" scope="component">
