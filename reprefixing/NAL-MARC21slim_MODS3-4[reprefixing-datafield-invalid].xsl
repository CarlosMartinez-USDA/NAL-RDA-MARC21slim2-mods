...
-->
        ...
        <xsl:if test="$typeOf008 = 'MP'">
            ...
            <xsl:choose>
                <xsl:when
                    test="$controlField008-25 = 'a' or $controlField008-25 = 'b' or $controlField008-25 = 'c' or 
                    marc:controlfield[@tag = 007][substring(text(), 1, 1) = 'a'][substring(text(), 2, 1) = 'j'] or
                    controlfield[@tag = 007][substring(text(), 1, 1) = 'a'][substring(text(), 2, 1) = 'j']">
                    <genre authority="marcgt">map</genre>
                </xsl:when>
                <xsl:when
                    test="$controlField008-25 = 'e' or
                    marc:controlfield[@tag = 007][substring(text(), 1, 1) = 'a'][substring(text(), 2, 1) = 'd'] or
                    controlfield[@tag = 007][substring(text(), 1, 1) = 'a'][substring(text(), 2, 1) = 'd']">
        ...
        <originInfo>
            ...
            <!-- unprefixed marc 250 and 260 -->
            <xsl:for-each
                select="datafield[(@tag = 260 or @tag = 250) and subfield[@code = 'a' or code = 'b' or @code = 'c' or code = 'g']]">
                <xsl:call-template name="z2xx880"/>
            </xsl:for-each>
        
            <xsl:variable name="length-of-008" select="string-length($controlField008)"/>
            <xsl:variable name="marcCountry">
                
            <xsl:choose>
                <xsl:when test="$length-of-008 &lt; 37">
                    <xsl:analyze-string select="substring($controlField008,1,18)" regex="(\d{{6}}).(\d{{0,8}})(\D+)">
                        <xsl:matching-substring>
            <xsl:matching-substring>
               
                            <xsl:value-of select="substring-before(regex-group(3), '|')"/>
                        </xsl:matching-substring>
                    </xsl:analyze-string>
                </xsl:when>
                <xsl:when test="regex-group(3) eq 'xx'"/>
        <xsl:when test="matches(regex-group(3),'[a-z]{2,3}') and not(regex-group(3) eq 'xx')">
            <xsl:value-of select="normalize-space(regex-group(3))"/> 
        </xsl:when>
        <xsl:when test="matches(regex-group(3), '\D{4,}')"/>
        <xsl:otherwise/>
   
                        <xsl:non-matching-substring>
                                    <xsl:variable name="marcPublicationCode"
                                        select="normalize-space(translate(replace($controlField008, '(\d{4,8}[a-z]\d{0,8})(\w+)', '$2'), '0123456789|# ', ''))"/>
...
<!--                          <xsl:when test="string-length($controlField008-1-15) = 15">true</xsl:when>-->
            
           <!-- <xsl:variable name="controlField008-1-20" select="substring($controlField008,1,20)"/>
                <xsl:variable name="marcCountryCode">
                <xsl:analyze-string select="$controlField008-1-20" regex="(/d{{4,8}}[a-z]/d{{0,8}})(\D{{2,4}})">
                    <xsl:matching-substring>
                        <xsl:value-of select="normalize-space(translate(regex-group(2),'0123456789|# ', ''))"/>
                    </xsl:matching-substring>
                    <xsl:non-matching-substring>
                        <xsl:variable name="marcPublicationCode"-->

            <!-- controlField008 "regex replaces then groups. 
                The result of it then is translated, and whitespace-normalized
            <xsl:variable name="controlField008-07-16">
                <xsl:sequence select="normalize-space(translate(replace(substring($controlField008,07,), '(s\d+[a-z]\d{0,8})(\D{3,4})(.*[a-z]\D+)[a-z]{3}\D+', '$2'), '0123456789|# ', ''))""
            </xsl:variable> 
            <xsl:variable name="controlField008-07-16_analyzed">
                <xsl:analyze-string select="$controlField008-07-16"
                    regex="([a-z]{{2}})|([a-z]{{2,3}})|([a-z]{{4,}})">-->
          <!--          <xsl:matching-substring>
                        <xsl:choose>
                            <xsl:when test="regex-group(1) eq 'xx'"/>
                            <xsl:when test="string-length(regex-group(2)) &lt;= 3 and string-length(regex-group(2)) &gt;= 2">
...
-->
	       ...
	       <xsl:if test="translate($marcCountryCode, '|', '') and matches($marcCountryCode, '[a-z]{2,3}')">
	            <place>
	                <!-\- marccountry code -\->
	                <placeTerm>
...
-->
    ...
    <xsl:template name="createNoteFrom500">
        <note>
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="f:capitalize-first(subfield[@code = 'a'])"/>
        </note>
    </xsl:template>
