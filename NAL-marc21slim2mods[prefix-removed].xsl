<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:strip-space elements="*"/>
	   ...
	   <!-- Maintenance note: For each revision, change the content of &lt;recordInfo&gt;&lt;recordOrigin&gt; to reflect the new revision number.
			...
			Revision 2.04 - Added full xpath to originInfo elements. cm3 2022/10/05
			Revision 2.03 - Added if tests to originInfo producer elements to avoid empty tags.  2022/11/08
			Revision 2.02 - analyze-string 008 to pull out 2-3 letter text and apply custom marc country conversion function
...
-->
        ...
        <originInfo>
            ...
            <xsl:variable name="MARCpublicationCode2">
                <xsl:analyze-string select="replace($controlField008,'(\d+[a-z]\d+)(\D{2,3})','$2')" regex="(\d+[a-z]\d+)([a-z]{{2,3}})">
                    ...
                    <xsl:non-matching-substring>
                        ...
                        <xsl:if test="starts-with($MARCpublicationCode,'0123455789')">
                            <xsl:value-of select="translate($MARCpublicationCode,'0123456789',' ')"/>
                            <place>
                                <placeTerm>
    ...
    -->
                                        ...
                                        <xsl:choose>
                                            <xsl:when test="matches(regex-group(3), '\d{8}')">
                                                ...
                                                <xsl:number value="substring(regex-group(3), 5, 6)"
                                             	...
                                             	<xsl:text>-</xsl:text>
                                                <!--DD-->
                                                <xsl:number value="substring(regex-group(3), 7, 8)"
                                                    format="01"/>
                                            </xsl:when>
            ...
            <!--1.94 -->
            <xsl:if test="$marcLeader7 = 'a' or $marcLeader7 = 'c' or $marcLeader7 = 'd' or $marcLeader7 = 'm' or $marcLeader7 = 'm' and ($marcLeader19 = 'a' or $marcLeader19 = 'b' or $marcLeader19 = 'c') or $marcLeader7 = 'm' and ($marcLeader19 = '#')">
            <xsl:for-each select="leader">
                <issuance>
                    <xsl:choose>
                        <xsl:when
                            test="$marcLeader7 = 'a' or $marcLeader7 = 'c' or $marcLeader7 = 'd' or $marcLeader7 = 'm'">monographic</xsl:when>
<!--                        <xsl:when test="$marcLeader7 = 'b'">continuing</xsl:when>-->
                        <xsl:when
                            test="$marcLeader7 = 'm' and ($marcLeader19 = 'a' or $marcLeader19 = 'b' or $marcLeader19 = 'c')">multipart monograph</xsl:when>
...
-->
        ...
        <!-- NAL subject authority codes agricola from 072 -->
     ...
     <xsl:if test="matches(.,'[A-Z]\d{3}')">
		...
		<recordInfo>
			...
			<!-- cm3 Revision 2.04 updated recordOrigin to reflect the XSLT filename used in transform cm 2022/12/08 -->
			<recordOrigin>
				        ...
				        <xsl:matching-substring>
				            <!-- issue is not present-->
				                        ...
				                        <xsl:if test="contains(regex-group(2), 'no.')">
				                            <detail type="issue">
				                                <number>
				                                    <xsl:value-of select="replace(regex-group(2), '(.+)(no. )(\d+)(.*)', '$3')"/>
				                                </number>
				                                <caption>no.</caption>
				                            </detail>
				                        </xsl:if>
    				                        <xsl:choose>
    				                            ...
    				                            <xsl:otherwise>
    				                                <xsl:if test="matches(.,'(.*p.)(\d{1,7})\-')">    				                                <extent unit="pages">
    				                                    <total>
    				                                        <xsl:value-of select="replace(regex-group(2), '(.*p.)(\d{1,7})\-', '$2')"/>
    				                                    </total>
    				                                </extent>
    				                                </xsl:if>
    				                            </xsl:otherwise>
    ...
    <xsl:template name="relatedIdentifierISSNandLocal">
	...
	<xsl:template name="relatedItem510">
	    ...
	    <xsl:call-template name="relatedIdentifierISSNandLocal"/>
