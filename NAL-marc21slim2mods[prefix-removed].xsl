<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<xd:doc>
		<xd:desc> Maintenance note: For each revision, change the content of &lt;recordInfo&gt;&lt;recordOrigin&gt; to reflect the new revision number.
			MARC21slim2MODS3-4 (Revision 1.98) 20141216
			REvision 2.04 cm3 2022 added if test to originInfo producer
			
			Revision 2.05 - controlField008-35-37replace, uses replace function and regex to pinpoint 3 letter strin cm3 2022/12/05
			Revision 2.04 - Added full xpath to originInfo elements cm3 2022/10/05
			Revision 2.03 - Added if tests to originInfo producer elemetns to avoid empty tags. 
...
-->
        ...
        <originInfo>
            ...
            <xsl:variable name="MARCpublicationCode">
                <xsl:analyze-string select="self::node()" regex="(\d+[a-z]\d+)([a-z]{{2,3}})">
    ...
    -->
        ...
        <xsl:for-each select="datafield[@tag = 264][@ind2 = 0]">
            <!-- Revision 2.03 cm3 2022 added if test to originInfo producer -->
            <originInfo displayLabel="producer">
                <!-- Template checks for altRepGroup - 880 $6 -->
                <!-- Revision 2.04 cm3 2022-12-05 added full xpath to originInfo elements -->
  ...
  <xsl:choose>
      <xsl:when test="not(ends-with($controlField008-35-37, '|'))">
            <language>
      ...
      <xsl:otherwise>
          <language>
              <languageTerm authority="iso639-2b" type="code">
                  <xsl:value-of select="substring($controlField008, 35, 3)"/>
              </languageTerm>
          </language>
      </xsl:otherwise>
  </xsl:choose>
