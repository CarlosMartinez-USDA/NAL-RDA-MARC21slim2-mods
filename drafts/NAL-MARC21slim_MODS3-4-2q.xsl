...
-->
	    ...
	    <originInfo>
	        ...
	        <!-- 2.02 cm3 -->
	        <xsl:template match="controlfield[@tag = '008']" mode="marcCountry">
	            <xsl:param name="controlfield008" as="xs:NMTOKEN"/>
	        </xsl:template>
	        <xsl:variable name="controlField008-07-16" select="translate(replace($controlField008,'(\d+[a-z]\d{0,8})(\D{3,4})(.*[a-z]\|\|)','$2'),'0123456789|',' ')"/>
	        <xsl:variable name="MARCpublicationCode">
	            <xsl:analyze-string select="$controlField008-07-16" regex="([x]{{2}})|([a-z]{{3}})">
	        ...
	        <!--<xsl:if test="translate($MARCpublicationCode, '|', '')">-->
	            ...
	            <place>
	                ...
	                <placeTerm>
	                    <xsl:attribute name="type">code</xsl:attribute>
	                    <xsl:attribute name="authority">countrymarc</xsl:attribute>
	                    <xsl:value-of select="$MARCpublicationCode"/>
	                </placeTerm>
